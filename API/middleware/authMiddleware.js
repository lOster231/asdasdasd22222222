 ...
 * Also checks if the user's email is verified for most protected routes.
 * The /me endpoint is an exception, allowing unverified access to fetch status.
...
const authMiddleware = (req, res, next) => {
    ...
    try {
        ...
        // 4. IMPORTANT CHANGE: Check if user's email is verified (for most routes).
        // EXCEPTION: If the request is for '/me', allow it through regardless of verification status.
        // This is so the frontend can retrieve the user's current verification status.
        // For other protected routes, verification is still enforced.
        if (req.path !== '/me' && !req.user.is_verified) {
