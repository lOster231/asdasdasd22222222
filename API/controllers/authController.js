...
exports.login = async (req, res, next) => {
    try {
        ...
        // console.log('Password provided:', password); // TEMPORARY: DO NOT LOG IN PRODUCTION
        // DEBUGGING LOGS: END
        ...
        if (error) {
            const errors = error.details.map(detail => detail.message);
            console.log('Login validation failed:', errors); // DEBUG
            return res.status(400).json({ message: 'Validation failed', errors: errors });
        }

        // 2. Find user by email
        const user = await userModel.findByEmail(email);
        console.log('User found in DB:', user ? user.email : 'None'); // DEBUG
        ...
        // console.log('Stored hashed password:', user.password); // DEBUG: TEMPORARY, DO NOT LOG IN PRODUCTION
        const isMatch = await bcrypt.compare(password, user.password);
        console.log('Password comparison result (isMatch):', isMatch); // DEBUG
        ...
        // IMPORTANT CHANGE (Applied in previous step): Do NOT return 403 here if not verified.
        // Instead, proceed to generate JWT, and the frontend will handle the unverified state.
        // The is_verified flag in the token payload will inform the frontend.

        // 5. Generate JWT token
        const tokenPayload = {
            id: user.id,
            email: user.email,
            is_verified: user.is_verified, // Ensure this is correctly included
        ...
        res.status(200).json({
            ...
            email: user.email,
            username: user.username,
            is_verified: user.is_verified, // Crucial for frontend logic
        ...
        console.error('Unhandled error in login controller:', error); // DEBUG
        next(error);
    }
};
...
exports.getAuthenticatedUser = async (req, res, next) => {
    try {
        const userId = req.user.id; // User ID is available from the decoded JWT
        const user = await userModel.findById(userId);
        ...
        // Return a subset of user data relevant for the frontend
        res.status(200).json({
            id: user.id,
            username: user.username,
            name: user.name,
            ...
            nationality: user.nationality,
            is_profile_public: user.is_profile_public,
            // Simulate based on is_verified, assuming it's equivalent to email_verified_at for the frontend logic.
...
exports.requestPasswordReset = async (req, res, next) => {
    try {
        ...
        // This link will now go to the frontend's ResetPasswordPage which will then redirect back to home with the token
...
/**
 * NEW: Validates a password reset token without invalidating it.
 * Used by the frontend's ResetPasswordPage to check token validity before redirecting.
 * @param {Object} req - Express request object.
 * @param {Object} res - Express response object.
 * @param {Function} next - Express next middleware function.
 */
exports.validateResetToken = async (req, res, next) => {
    try {
        ...
        // Find the user and check token validity in the database (reset_token AND not expired)
        ...
        // Token is valid; return success and user ID (or other minimal info)
 ...
 * This is where the token is CONSUMED (invalidated).
 * @param {Object} req - Express request object.
 * @param {Object} res - Express response object.
 * @param {Function} next - Express next middleware function.
 */
